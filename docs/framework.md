...
`用户/系统入口 -> APISIX -> 门户工作台/BFF -> Agent/Workflow 编排 -> MCP / A2A 协议层 -> RAG/工具/远程Agent -> LiteLLM -> vLLM/Qwen`
...
### 14. OpenTelemetry + Prometheus + Grafana + Loki
...
6. 如需工具、上下文或外部资源，请优先通过 `MCP` 调用相关能力
7. 如需跨 Agent 协作或远程能力编排，通过 `A2A` 调用远程 Agent
